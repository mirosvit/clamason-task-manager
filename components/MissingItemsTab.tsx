...
import { Task } from './PartSearchScreen';
...
const MissingItemsTab: React.FC<MissingItemsTabProps> = ({ tasks, onDeleteMissingItem, hasPermission }) => {
    ...
    const missingTasks = useMemo(() => {
        return tasks.filter(t => t.isMissing).sort((a, b) => {
    ...
    const filteredItems = useMemo(() => {
        ...
        return missingTasks.filter(t => 
            (t.partNumber && t.partNumber.toLowerCase().includes(q)) ||
            (t.workplace && t.workplace.toLowerCase().includes(q)) ||
            (t.missingReportedBy && t.missingReportedBy.toLowerCase().includes(q)) ||
            (t.missingReason && t.missingReason.toLowerCase().includes(q))
        );
